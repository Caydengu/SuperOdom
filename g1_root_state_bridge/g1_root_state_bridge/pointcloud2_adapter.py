...
def decode_livox_pointcloud2(
    ...
    *,
    ...
    if time_unit not in {"nanoseconds", "seconds"}:
        raise PointCloud2ContractError("PointCloud2 time unit must be nanoseconds or seconds")
    fields = {str(field.name): field for field in message.fields}
    missing = sorted({"x", "y", "z", "time"} - fields.keys())
    if missing:
        raise PointCloud2ContractError(f"PointCloud2 is missing fields: {', '.join(missing)}")
    endian = ">" if bool(message.is_bigendian) else "<"
    columns = []
    for name in ("x", "y", "z", "time"):
        field = fields[name]
        if int(field.datatype) != 7 or int(field.count) != 1:
            raise PointCloud2ContractError(f"PointCloud2 {name} must be one float32 field")
        columns.append(
            np.ndarray(
    ...
    spatially_admitted = finite & (squared_range >= minimum_range_m**2) & (
    ...
    if indices.size > maximum_points:
        indices = indices[np.linspace(0, indices.size - 1, maximum_points, dtype=np.int64)]
    if indices.size == 0:
...
def header_time_ns(message: Any) -> int:
    value = int(message.header.stamp.sec) * 1_000_000_000 + int(message.header.stamp.nanosec)
    if value <= 0:
